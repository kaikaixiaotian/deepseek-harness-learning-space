...
8. **Viz-path locale gate (read-mode):** every `iframe src` and `.viz-open` href inside a chapter uses the workspace's locale-mapped demo dir (zh: `./演示/…`, en: `./viz/…`), and the demo files were WRITTEN into that same dir (verify with listDir or a Read of one target). A zh chapter referencing `./viz/` (or vice versa) shows the "演示文件缺失" placeholder in the learning space — fix the srcs, not the file layout.
9. **Contamination guard:** the HTML must not DEFINE any `--dsw-*` / `--dsh-*` custom properties — consuming them via `var(--dsw-alias-*, fallback)` is expected and required (theme bridge), but defining them would override the host theme. Also no `--vscode-` / `icube-theme-variables` strings — those indicate IDE/editor CSS was accidentally pasted into the file (a real incident added ~1900 junk style lines to a generated chapter). Strip or regenerate.
