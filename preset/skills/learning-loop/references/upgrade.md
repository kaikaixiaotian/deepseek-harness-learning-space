...
- New chapters/quizzes → the **dsh token skeleton** (v2): colors consume `--dsw-alias-*` with the dsh static palette as fallback (old files keep their own fallback CSS and keep working), the accent is the `state-business-*` blue family, and the `<html>` may carry a third host class `ll-glass` (glass themes like ui-aqua — translucent canvas so the host card's glass and ambient stay visible) alongside `ll-dark`/`ll-light`.
